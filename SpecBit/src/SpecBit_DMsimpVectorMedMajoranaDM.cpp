...
#include "gambit/Models/SimpleSpectra/DMsimpVectorMedMajoranaDMSimpleSpec.hpp"
...
namespace Gambit
{
  ...
  namespace SpecBit
  {
    ...
    void get_DMsimpVectorMedMajoranaDM_spectrum(Spectrum& result)
    {
      namespace myPipe = Pipes::get_DMsimpVectorMedMajoranaDM_spectrum;
      ...
      SLHAea_add(slha, "DMINPUTS", 1, *myPipe::Param["gAXm"]);
      SLHAea_add(slha, "DMINPUTS", 2, *myPipe::Param["gVq"]);
      ...
      SLHAea_add(slha, "MASS", 5000522, *myPipe::Param["MXm"]);
      ...
      result = spectrum_from_SLHAea<Gambit::Models::DMsimpVectorMedMajoranaDMSimpleSpec, SLHAstruct>(slha,slha,mass_cut,mass_ratio_cut);
    }
    
    
    // Declaration: print spectrum out
    void fill_map_from_DMsimpVectorMedMajoranaDM_spectrum(std::map<std::string,double>&, const Spectrum&);
    
    void get_DMsimpVectorMedMajoranaDM_spectrum_as_map(std::map<std::string,double>& specmap)
    {
      namespace myPipe = Pipes::get_DMsimpVectorMedMajoranaDM_spectrum_as_map;
      const Spectrum& spec(*myPipe::Dep::DMsimpVectorMedMajoranaDM_spectrum);
      fill_map_from_DMsimpVectorMedMajoranaDM_spectrum(specmap, spec);
    }
    
    void fill_map_from_DMsimpVectorMedMajoranaDM_spectrum(std::map<std::string, double>& specmap, const Spectrum& spec)
    {
      /// Use SpectrumContents routines to automate
      static const SpectrumContents::DMsimpVectorMedMajoranaDM contents;
      ...
      for(std::vector<SpectrumParameter>::const_iterator it = required_parameters.begin(); it != required_parameters.end(); ++it)
      {
        ...
        else
        {
          ...
          errmsg << "Invalid parameter received while converting DMsimpVectorMedMajoranaDM_spectrum to map of strings!";
    ...
    void Unitarity_Bound_DMsimpVectorMedMajoranaDM(double& result)
    {
    
      using namespace Pipes::Unitarity_Bound_DMsimpVectorMedMajoranaDM;
