...
#include "gambit/Models/SimpleSpectra/DMsimpVectorMedScalarDMSimpleSpec.hpp"
...
namespace Gambit
{
  ...
  namespace SpecBit
  {
    ...
    void get_DMsimpVectorMedScalarDM_spectrum(Spectrum& result)
    {
      namespace myPipe = Pipes::get_DMsimpVectorMedScalarDM_spectrum;
      ...
      SLHAea_add(slha, "DMINPUTS", 1, *myPipe::Param["gVXc"]);
      ...
      SLHAea_add(slha, "MASS", 5000520, *myPipe::Param["MXc"]);
      ...
      result = spectrum_from_SLHAea<Gambit::Models::DMsimpVectorMedScalarDMSimpleSpec, SLHAstruct>(slha,slha,mass_cut,mass_ratio_cut);
    }
    
    
    // Declaration: print spectrum out
    void fill_map_from_DMsimpVectorMedScalarDM_spectrum(std::map<std::string,double>&, const Spectrum&);
    
    void get_DMsimpVectorMedScalarDM_spectrum_as_map(std::map<std::string,double>& specmap)
    {
      namespace myPipe = Pipes::get_DMsimpVectorMedScalarDM_spectrum_as_map;
      const Spectrum& spec(*myPipe::Dep::DMsimpVectorMedScalarDM_spectrum);
      fill_map_from_DMsimpVectorMedScalarDM_spectrum(specmap, spec);
    }
    
    void fill_map_from_DMsimpVectorMedScalarDM_spectrum(std::map<std::string, double>& specmap, const Spectrum& spec)
    {
      /// Use SpectrumContents routines to automate
      static const SpectrumContents::DMsimpVectorMedScalarDM contents;
      ...
      for(std::vector<SpectrumParameter>::const_iterator it = required_parameters.begin(); it != required_parameters.end(); ++it)
      {
        ...
        else
        {
          ...
          errmsg << "Invalid parameter received while converting DMsimpVectorMedScalarDM_spectrum to map of strings!";
