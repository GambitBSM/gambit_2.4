...
# Plotting script for histograms generating by enabling Perf_Plot features in Analysis_ATLAS_8TeV_2bStop_20invfb.cpp
# Code by Kelton Whiteaker

# cross-sections for the different Signal Models used in the histos
xsect_250 = 5575.96
xsect_300 = 1996.08
xsect_500 = 85.5847
...
if len(sys.argv) < 3:
  ...
  print('You can normalize both histos to integrate to 1 by providing a 4th argument')
  exit()

plotname = sys.argv[1]
process = sys.argv[2]
normit = len(sys.argv) > 3  # normalize so both histos integrate to 1
if plotname == "mct-3a":
  xsect = xsect_500
  var = 'mCT'
  ...
  binwidth = 25.0
  pltrange = (0,500)
#  atlas_data = np.array([0,0.7,2.0,2.1,3.4,5.4,7.1,8.5,7.0,8.7,7.8,8.1,7.2,7.3,7.2,4.6,3.9,3.8,2.1,1.0])  # process 1, copied down by eye
  ...
  title = r'Distribution of $m_{CT}$ after all SRA cuts'+'\n'+r'except $m_{CT}$ cuts, Process '+process
elif plotname == "mct-aux2a":
  xsect = xsect_500
  var = 'mCT'
  f = h5py.File('process'+process+'/ATLAS_8TeV_2bStop_20invfb_SRA_all-cuts.hdf5', 'r')
  dset = f[u'mct']
  nbins = 16
  binwidth = 25.0
  pltrange = (150,550)
  ...
  title = r'Distribution of $m_{CT}$ after all SRA cuts'+'\n'+r'up to and incl $m_{CT}>150$ GeV, Process '+process
  ...
  title = r'Distribution of $m_{bb}$ after all SRA cuts'+'\n'+r'up to and incl $m_{CT}>150$ GeV, except $m_{bb}>200$ GeV, Process '+process
  ...
  title = r'Distribution of $H_{T3}$ after all SRB cuts'+'\n'+r'except final $H_{T3}$ cut, Process '+process
else:
  print('You need to specify which plot to replicate!\nYour options are: mct-3a, mbb-3b, met-4a, ht3-4b, mct-aux2a, met-aux2b\nYou also need to specify process: "1" or "2"')
  exit()
...
evtnum = 50000.0  # number of generated events
